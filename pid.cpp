...
//    if(timeChange > 10000 || timeChange <= 0)
//        timeChange = 0.00001;
...
void PID::report()
{
    std::cout << " Az: " << counterAz << " " << targetAz * 180 / 3.141592654 << " " << angleAz * 180 / 3.141592654 << "         "
...
void PID::motorControl(double speed, int axis, int motorLead1, int motorLead2, bool forward)
{
    ...
    if(!forward)
    {
        ...
        set_PWM_dutycycle(piNumber, motorLead1, (int)(difference * 512));
    }
    else
    {
        set_PWM_dutycycle(piNumber, motorLead1, 0);
        set_PWM_dutycycle(piNumber, motorLead2, (int)(difference * 512));
    }
...
void PID::run()
{
    ...
    while(programRunning)
    {
        ...
        if(MainWindow::tracking)
        {
            ...
            std::cout << dtime.second + dtime.fraction / 1e6 << "   " << targetAz - angleAz << std::endl;
